<?php
	...
	if((isset($obj->load_farmer)) == '1' && (isset($obj->load_farmer)))
	{
		...
		if($page != "" && $per_page != "")	
		{
			...
			$sql_load_data  .= " FROM `tbl_farmers` AS tf INNER JOIN `tbl_applicant_phone` AS tp ON tf.fm_id = tp.fm_id ";
			...
			if($hid_page_type == 'smartphone')
			{
				$sql_load_data  .= " AND tp.f5_phonetype = 'smartphone' ";	
			}

			if($hid_page_type == 'featurephone')
			{
				$sql_load_data 	.= " AND tp.f5_phonetype = 'featurephone' ";
			}
			...
			{		
				...
				while($row_load_data = mysqli_fetch_array($result_load_data))
				{
					...
					$cat_data .= '<tr>';				
						...
						if($hid_page_type == 'smartphone') 
						{ 
							$cat_data	.= 'Smart Phone'; 
						} 
						elseif($hid_page_type == 'featurephone') 
						{ 
							$cat_data 	.= 'Featured Phone'; 
						};
						$cat_data .= '</td>';	//<!-- Phone Type -->
