<?php
	...
	if((isset($obj->load_farmer)) == '1' && (isset($obj->load_farmer)))
	{
		...
		if($page != "" && $per_page != "")	
		{
			...
			$sql_load_data  .= " FROM `tbl_farmers` AS tf ";

			if($hid_page_type == 'total_com')
			{
				$sql_load_data  .= " INNER JOIN `tbl_points` AS tp ON tf.fm_id = tp.fm_id ";				
			}
			...
			if($hid_page_type == 'total_com')
			{
				$sql_load_data  .= " AND tp.pt_frm1 != '' AND tp.pt_frm2 != '' AND tp.pt_frm3 != '' AND tp.pt_frm5 != '' AND tp.pt_frm7 != '' ";
				$sql_load_data  .= " AND tp.pt_frm8 != '' AND tp.pt_frm9 != '' AND tp.pt_frm10 != '' AND tp.pt_frm11 != '' AND tp.pt_frm12 != '' ";
				$sql_load_data  .= " AND tp.pt_frm13 != '' AND tp.pt_frm14 != '' ";	
			}

			if($hid_page_type == 'total_incom')
			{
				$sql_load_data 	.= " AND tf.fm_id NOT IN (SELECT tf.fm_id ";
				$sql_load_data 	.= " FROM `tbl_farmers` AS tf INNER JOIN `tbl_points` AS tp ";
				$sql_load_data 	.= " 	ON tf.fm_id = tp.fm_id ";
				$sql_load_data 	.= " WHERE tp.pt_frm1 != '' AND tp.pt_frm2 != '' AND tp.pt_frm3 != '' AND tp.pt_frm5 != '' AND tp.pt_frm7 != '' ";
				$sql_load_data 	.= " 	AND tp.pt_frm8 != '' AND tp.pt_frm9 != '' AND tp.pt_frm10 != '' AND tp.pt_frm11 != '' ";
				$sql_load_data 	.= " 	AND tp.pt_frm12 != '' AND tp.pt_frm13 != '' AND tp.pt_frm14 != '') ";
			}
			...
			{		
				...
				while($row_load_data = mysqli_fetch_array($result_load_data))
				{
					...
					$Aadhaar_image            = getImage($row_load_data['fm_id'], 'Aadhar');
					$Profile_Photo_image      = getImage($row_load_data['fm_id'], 'Profile Photo');
					$saat_bara_image          = getImage($row_load_data['fm_id'], '7/12');
					$land_Registration_image  = getImage($row_load_data['fm_id'], 'land Registration');
					$land_Valuation_image     = getImage($row_load_data['fm_id'], 'land Valuation');
					$Pancard_image            = getImage($row_load_data['fm_id'], 'Pancard');
					$Soil_test_document_image = getImage($row_load_data['fm_id'], 'Soil test document');
					$Kisan_Credit_Card_image  = getImage($row_load_data['fm_id'], 'Kisan Credit Card');
					...
					$cat_data .= '<tr>';				
						...
						$cat_data .= '<td style="text-align:center;">';
							$cat_data .= '<a href="javascript:void(0);" id="view_uploads'.$row_load_data['fm_id'].'" onclick="getDisplayViewUploadDiv(\'farmers_'.$row_load_data['fm_id'].'\')" class="btn btn-primary">';
								$cat_data .= 'View Uploads';
							$cat_data .= '</a>';
						$cat_data .= '</td>';										//<!-- View Uploads -->
					$cat_data .= '</tr>';
					$cat_data .= '<tr>';
						$cat_data .= '<td colspan="10" style="text-align:center;">';
							$cat_data .= '<div id="farmers_'.$row_load_data['fm_id'].'" style="border:1px solid #000;display:none;">';
								// Have to show all the uploads of the respective farmer
								
								$cat_data .= '<table class="table table-bordered dataTable" style="width:100%;text-align:center">';
									$cat_data .= '<tr>';
										$cat_data .= '<th style="text-align:center;">Aadhar</th>';				// <!-- Aadhar -->
										$cat_data .= '<th style="text-align:center;">7/12</th>';				// <!-- 7/12 -->
										$cat_data .= '<th style="text-align:center;">land Registration</th>';	// <!-- land Registration -->
										$cat_data .= '<th style="text-align:center;">land Valuation</th>'; 	// <!-- land Valuation -->
										$cat_data .= '<th style="text-align:center;">Pancard</th>';			// <!-- Pancard -->
										$cat_data .= '<th style="text-align:center;">Soil test document</th>'; // <!-- Soil test document -->
										$cat_data .= '<th style="text-align:center;">Kisan Credit Card</th>';	// <!-- Kisan Credit Card -->
									$cat_data .= '</tr>';
									$cat_data .= '<tr>';
										$cat_data .= '<td style="text-align:center;">'; 
											if($Aadhaar_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$Aadhaar_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
										...
										$cat_data .= '<td style="text-align:center;">'; 
											if($saat_bara_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$saat_bara_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
										...
										$cat_data .= '</td>';				// <!-- 7/12 -->
										$cat_data .= '<td style="text-align:center;">'; 
											if($land_Registration_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$land_Registration_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
											}
										$cat_data .= '</td>';	// <!-- land Registration -->
										$cat_data .= '<td style="text-align:center;">'; 
											if($land_Valuation_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$land_Valuation_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
											}
										$cat_data .= '</td>'; 	// <!-- land Valuation -->
										$cat_data .= '<td style="text-align:center;">'; 
											if($Pancard_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$Pancard_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
										...
										$cat_data .= '</td>';			// <!-- Pancard -->
										$cat_data .= '<td style="text-align:center;">'; 
											if($Soil_test_document_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$Soil_test_document_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
											}
										$cat_data .= '</td>'; // <!-- Soil test document -->
										$cat_data .= '<td style="text-align:center;">'; 
											if($Kisan_Credit_Card_image != '')
											{
												$cat_data .= '<img src="data/'.$row_load_data['fm_id'].'/'.$Kisan_Credit_Card_image.'" alt="'.$row_load_data['fm_name'].'" width="70" height="70">';
											}
											else
											{
												$cat_data .= '<img src="images/person.jpg" alt="No Image" width="70" height="70">';
											}
										$cat_data .= '</td>';	// <!-- Kisan Credit Card -->
									$cat_data .= '</tr>';
								$cat_data .= '</table>';
							$cat_data .= '</div>';
						$cat_data .= '</td>';
