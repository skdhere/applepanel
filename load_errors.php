<?php
	...
	if((isset($obj->load_fpo)) == "1" && isset($obj->load_fpo))
	{
		...
		if($page != "" && $per_page != "")	
		{
			...
			$sql_load_data  = " select * from tbl_upload_errors WHERE 1 = 1 ";
			if($search_text != "")
			{
				$sql_load_data .= " and (fm_id like '%".$search_text."%' or error_msg like '%".$search_text."%' ";
				$sql_load_data .= " or error_data like '%".$search_text."%') ";	
			}
			// quit($sql_load_data);
			...
			{		
				...
				$fpo_data .= '<thead>';
					$fpo_data .= '<tr>';
						...
						$fpo_data .= '<th>Fm Id</th>';
						$fpo_data .= '<th>Error Msg</th>';
						$fpo_data .= '<th>Error Data</th>';
						$fpo_data .= '<th>Edit</th>';
					$fpo_data .= '</tr>';
				$fpo_data .= '</thead>';
				$fpo_data .= '<tbody>';
				while($row_load_data = mysqli_fetch_array($result_load_data))
				{
					$fpo_data .= '<tr>';				
						...
						$fpo_data .= '<td>';
							$fpo_data .= $row_load_data['fm_id'];
						$fpo_data .= '</td>';	//<!-- Farmer Name -->
						$fpo_data .= '<td>'.$row_load_data['error_msg'].'</td>';	//<!-- Mobile Number -->
						$fpo_data .= '<td>'.substr($row_load_data['error_data'],0,200).'...</td>';	//<!-- Email -->
							//<!-- Status -->
						
						$fpo_data .= '<td style="text-align:center;">';
							$fpo_data .= '<a href="add_error_data.php?pag=fpo&id='.$row_load_data['id'].'" class="btn btn-primary">Edit</a>';
