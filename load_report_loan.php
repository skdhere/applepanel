<?php
	...
	if((isset($obj->load_farmer)) == '1' && (isset($obj->load_farmer)))
	{
		...
		if($page != "" && $per_page != "")	
		{
			...
			$sql_load_data  = " SELECT tf.*, tpd.f1_required_loan_amt ";
			$sql_load_data  .= " FROM tbl_farmers AS tf INNER JOIN tbl_personal_detail AS tpd ";
			$sql_load_data  .= " 	ON tf.fm_id = tpd.fm_id ";
			$sql_load_data  .= " WHERE tpd.f1_required_loan = 'yes' ";
			...
			{		
				...
				$cat_data .= '<thead>';
					$cat_data .= '<tr>';
						...
						$cat_data .= '<th>Required Loan Amount</th>';
				...
				while($row_load_data = mysqli_fetch_array($result_load_data))
				{
					...
					$cat_data .= '<tr>';				
						...
						$cat_data .= '<td>'.$row_load_data['f1_required_loan_amt'].'</td>';	//<!-- Required Loan Amount -->
