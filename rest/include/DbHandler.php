...
class DbHandler
{
    ...
    */
    ...
    public function crateKycKnowladgeEntry($data, $_user_id)
    {
        ...
        $stmt->bind_param("iisssssssssissss", $data['fm_caid'], $data['fm_id'], $data['f2_edudetail'], $data['f2_proficiency'], $data['f2_participation'], $data['f2_typeprog'], $data['f2_nameprog'], $data['f2_dateprog'], $data['f2_durprog'], $data['f2_condprog'], $data['f2_cropprog'], $f2_status, $f2_section_id, $data['f2_points'], $dat, $_user_id);
    ...
    public function updateKycKnowladgeEntry($data, $_user_id, $farmer_id)
    {
        ...
        $stmt->bind_param("sssssssssissssi", $data['f2_edudetail'], $data['f2_proficiency'], $data['f2_participation'], $data['f2_typeprog'], $data['f2_nameprog'], $data['f2_dateprog'], $data['f2_durprog'], $data['f2_condprog'], $data['f2_cropprog'], '1', '', $data['f2_points'], $dat, $_user_id);
        $result = $stmt->execute();
        $stmt->close();
        // Check for successful insertion
        if ($result) {
    ...
    public function updateKycSpouseEntry($data, $_user_id)
    {
        // Update query
        $dat = date("Y-m-d H:i:s"); // Get Current DateTime for Modified Date
